def generate_stars(db_name, decisions_provided):
    ...
    def get_stellarcharsv():
        ...
        for line in open(r"tables\Star Characteristics V.txt"):
            ...
            temp_stellarcharsv[data[0]] = dict(
                zip(('temperature', 'luminosity', 'mass', 'radius', 'lifespan'), data[1:]))
    ...
    def get_stellarcharsiii():
        ...
        for line in open(r"tables\Star Characteristics III.txt"):
            data = line.strip().split(',')
            temp_stellarcharsiii[data[0]] = dict(
                zip(('temperature', 'luminosity', 'mass', 'radius', 'lifespan'), data[1:]))
    ...
    def get_world_type_table():
        ...
        for line in open(r"tables\World Type Table.txt"):
            data = line.strip().split(',')
            temp_world_type[data[0]] = dict(
                zip(('Inner Zone', 'Life Zone', 'Middle Zone', 'Outer Zone', 'Forbidden'), data[1:]))
    ...
    def populate_stellar_orbit_info(location, stellar_dict_list):
        ...
        for ix_star, star_dict in enumerate(stellar_dict_list):
            ...
            if r1 > r2:
                orbital_inner_limit = r1
            else:
                orbital_inner_limit = r2
    ...
    def get_climate(temperature, world_type):
        ...
        if world_type[0] == 'O':
            if temperature <= 238:
                c_climate = 'Uninhabitable (Frigid)'
            elif temperature <= 249:
                c_climate = 'Frozen'
            elif temperature <= 260:
                c_climate = 'Very Cold'
            elif temperature <= 272:
                c_climate = 'Cold'
            elif temperature <= 283:
                c_climate = 'Chilly'
            elif temperature <= 294:
                c_climate = 'Cool'
            elif temperature <= 302:
                c_climate = 'Earth-normal'
            elif temperature <= 308:
                c_climate = 'Warm'
            elif temperature <= 313:
                c_climate = 'Tropical'
            elif temperature <= 319:
                c_climate = 'Hot'
            elif temperature <= 324:
                c_climate = 'Very Hot'
            else:
                c_climate = 'Uninhabitable (Torrid)'
        return c_climate
                    ...
                    if forbidden_planet == False:
                        ...
                        if natural_moons > 0:
                            for m in range(1, natural_moons + 1):
                                ...
                                if moon_density == 0:
                                    ...
                                    moon_size = round(
                                        (6370 * (integer_root(3, (moon_mass / moon_density)))) * 2 / 1609.3, 0)
                        ...
                        if impact_moons > 0:
                            ...
                            if moon_density == 0:
                                ...
                                moon_size = round((6370 * (integer_root(3, (moon_mass / moon_density)))) * 2 / 1609.3,
                                                  0)
    ...
    def populate_stellar_tables(stellar_list):
        ...
        for star in stellar_list:
            c.execute("INSERT INTO stellar_bodies (location, companion_class, luminosity_class,\
                ...
                str(star["luminosity_class"].name),
                star["spectral_type"],
                star["age"],
                star["temperature"],
                star["luminosity"],
            ...
            for y in range(1, 41):
                ...
                if rollgen >= LIKELIHOOD:
                    ...
                    for pc in range(0, primary_companions + 1):  # one loop for each non subcompanion star
                        if pc == 0:
                            stellar_dict = populate_stellar_dict(parsec, pc, 0, primary_companions,
                                                                 False, conn=conn)
                            stellar_dict_list.append(stellar_dict)
                        else:
                            stellar_dict = populate_stellar_dict(parsec, pc, stellar_dict_list[0], primary_companions,
                                                                 False, conn=conn)
                            stellar_dict_list.append(stellar_dict)

                            if stellar_dict['companions'] > 0:
                                stellar_dict = populate_stellar_dict(parsec, pc, stellar_dict, primary_companions,
                                                                     True, conn=conn)
                                stellar_dict_list.append(stellar_dict)
